...
df3.loc[(df3['天然气(标准煤)'].isnull().T.any()),'是否补数据']='1'
...
# print(df3)
df4=df3.groupby(["企业名称","统一信用代号","ym","行业","地市","电(千瓦时)","煤(标准煤)","油(标准煤)","天然气(标准煤)","热力(标准煤)","综合能耗(标准煤)","是否补数据"],dropna=False,).agg({"能耗等级":"max","pq":"sum"}).reset_index()
df3=df4[['企业名称','统一信用代号','ym','行业','地市','能耗等级','pq','电(千瓦时)','煤(标准煤)','油(标准煤)','天然气(标准煤)','热力(标准煤)','综合能耗(标准煤)','是否补数据']]
df3.to_excel(r'D:\Users\PC\Desktop\test\结果3.xlsx', index=False)
